...
def get_dataloaders(dataset, batch, batch_unsup, dataroot):
    ...
    if dataset in ["cifar10", "cifar100"]:
        if dataset == "cifar10":
            ...
            unsup_trainset = torchvision.datasets.CIFAR10(
            ...
            unsup_trainset = torchvision.datasets.CIFAR100(
                root=dataroot, train=True, download=True, transform=None
            )
        ...
        otherset = UnsupervisedDataset(
            otherset, transform_valid, autoaug, cutout=C.get()["cutout"]
        )
    else:
    ...
    trainloader = torch.utils.data.DataLoader(
        ...
        num_workers=8,
        pin_memory=True,
        sampler=StratifiedSampler(trainset.train_labels),
        drop_last=True,
    )

    unsuploader = torch.utils.data.DataLoader(
        otherset,
        batch_size=batch_unsup,
        shuffle=True,
        num_workers=8,
        pin_memory=True,
        sampler=None,
...
class UnsupervisedDataset(Dataset):
    def __init__(self, dataset, transform_default, transform_aug, cutout=0):
        ...
        self.transform_cutout = CutoutDefault(
            cutout
        )  # issue 4 : https://github.com/ildoonet/unsupervised-data-augmentation/issues/4
